...
void loop() {
  ...
  if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
    ...
    // reading first 2 bytes of 0xCD message raw data for current
    else if(canMsg.can_id == 0xCD){
      // current_raw is the current sensor voltage
      current_raw = canMsg.data[1];
      current_raw <<= 8;
      current_raw = current_raw | canMsg.data[2];
      
      current = (15.0 / 65535.0)* current_raw;// Read the voltage from CAN raw values
    }

    // reading first 2 bytes  0xEF message of raw data for Wheel Speed
    else if(canMsg.can_id == 0xEF){
      speed_raw = canMsg.data[1];
      speed_raw <<= 8;
      speed_raw = speed_raw | canMsg.data[2];
  
      wheelspeed = (100.0/65535.0) * speed_raw;
    }
